/* erlusb-manager.c - Erlang interface to libusb device manager
 * Copyright (C) 2006,2009,2010 Hans Ulrich Niedermann <hun@n-dimensional.de>
 *
...
int main() {
  ...
  while ((readlen = read_cmd(rbuf, sizeof(rbuf))) > 0) {
    ...
    if (0) {
      ...
      free(packet);
    } else if (strncmp(atom, "get_device_list", 16) == 0) {
